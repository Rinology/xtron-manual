...
* **사진 넣기**: 용량 관리를 위해 이미지는 클라우드에 올리고 주소만 가져옵니다.
  ```markdown
  ![사진설명글](https://이미지주소.jpg)
  ```
  👉 **결과 및 크기 제한:** 이미지는 자동으로 모서리가 둥글게(8px) 깎이고 테두리가 생깁니다.
  특히 **가로 최대 600px, 세로 최대 600px**로 자동 제한되므로, 해상도가 너무 크거나 세로로 긴 사진을 넣어도 화면을 뚫고 나가지 않고 **원본 비율을 유지하며 안전하게 축소**되어 표시됩니다.
